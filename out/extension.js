...
const treeSitterService_js_1 = require("./ast/treeSitterService.js");
function activate(context) {
    ...
    treeSitterService_js_1.TreeSitterService.setWasmDir(path.join(context.extensionPath, 'parsers'));
    treeSitterService_js_1.TreeSitterService.getInstance().initialize().then(() => {
        if (treeSitterService_js_1.TreeSitterService.getInstance().isInitialized()) {
            logger_js_1.logger.info('[Extension]', 'Tree-sitter AST initialized (languages: typescript, javascript, python, go)');
        }
        else {
            logger_js_1.logger.warn('[Extension]', 'Tree-sitter AST not available — WASM files missing or load failed');
        }
    }).catch(() => {
        logger_js_1.logger.warn('[Extension]', 'Tree-sitter initialization failed — AST features will be disabled');
    });
...
`;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('tokamak.configureMcp', async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage('No workspace folder open');
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('tokamak.initRules', async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage('No workspace folder open');
            return;
...
`;
        try {
            ...
            await vscode.workspace.fs.writeFile(vscode.Uri.joinPath(rulesFolder, 'ts-conventions.md'), Buffer.from(defaultRule, 'utf8'));
