...
var ChatPanel = class _ChatPanel {
  ...
  saveChatHistory() {
    if (_ChatPanel.extensionContext) {
      _ChatPanel.extensionContext.workspaceState.update("tokamak.chatHistory", this.chatHistory);
      _ChatPanel.extensionContext.workspaceState.update("tokamak.chatMode", this.currentMode);
    }
  }
  restoreChatHistory() {
    if (_ChatPanel.extensionContext) {
      const savedHistory = _ChatPanel.extensionContext.workspaceState.get("tokamak.chatHistory");
      const savedMode = _ChatPanel.extensionContext.workspaceState.get("tokamak.chatMode");
  ...
  async searchFiles(query) {
    ...
    try {
      ...
      const results = files.map((file) => ({
        path: vscode2.workspace.asRelativePath(file),
  ...
  async openFile(path) {
    try {
      ...
      vscode2.window.showErrorMessage(`Could not open file: ${path}`);
    }
  }
  ...
  async getFileContent(relativePath) {
    try {
      ...
      const fileUri = vscode2.Uri.joinPath(workspaceFolder.uri, relativePath);
...
Additional context: ${remainingText}` : slashCommand.prompt;
    ...
    const displayText = attachedFiles.length > 0 ? `${text}

\u{1F4CE} ${attachedFiles.join(", ")}` : text;
