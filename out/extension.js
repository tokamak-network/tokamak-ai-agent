...
var ChatPanel = class _ChatPanel {
    ...
    switch (message.command) {
      ...
      case "getSessions":
        this.panel.webview.postMessage({
          ...
          sessions: this.sessions.map((s2) => ({ id: s2.id, title: s2.title, timestamp: s2.timestamp })),
  ...
  async handleModelChange(model) {
    ...
    vscode2.window.showInformationMessage(`Model changed to: ${model}`);
  }
  async searchFiles(query) {
    ...
    try {
      ...
      for (const file of files) {
        const relPath = vscode2.workspace.asRelativePath(file);
        results.push({
          path: relPath,
    ...
    try {
      ...
      if (workspaceFolder) {
        ...
        if (stat.type === vscode2.FileType.Directory) {
          await vscode2.commands.executeCommand("revealInExplorer", fileUri);
        } else {
          await vscode2.window.showTextDocument(fileUri);
        }
      }
    } catch {
      vscode2.window.showErrorMessage(`Could not open: ${path}`);
    }
  }
  async resolveFilePath(uriString) {
    try {
      ...
      if (filePath.startsWith("file://")) {
        const uri = vscode2.Uri.parse(filePath);
        filePath = uri.fsPath;
      }
      ...
      const fileUri = vscode2.Uri.joinPath(workspaceFolder.uri, filePath);
      try {
        const stat = await vscode2.workspace.fs.stat(fileUri);
        if (stat.type === vscode2.FileType.File || stat.type === vscode2.FileType.Directory) {
          const fileName = filePath.split(/[/\\]/).pop() || filePath;
          this.panel.webview.postMessage({
            command: "fileDropped",
            path: filePath,
            name: fileName,
            isDir: stat.type === vscode2.FileType.Directory
          });
        }
      } catch {
      }
  ...
  async getFileContent(relativePath) {
    try {
      ...
      const uri = vscode2.Uri.joinPath(workspaceFolder.uri, relativePath);
      const stat = await vscode2.workspace.fs.stat(uri);
      if (stat.type === vscode2.FileType.Directory) {
        const files = await vscode2.workspace.findFiles(
          new vscode2.RelativePattern(uri, "**/*"),
          "**/node_modules/**",
          100
        );
        let structure = `
...
`;
        const folderTree = [];
        for (const file of files) {
          folderTree.push(`- ${vscode2.workspace.asRelativePath(file)}`);
        }
...
`;
  ...
  getEditorContext() {
    ...
    const workspaceFolder = vscode2.workspace.workspaceFolders?.[0];
    const relativePath = workspaceFolder ? vscode2.workspace.asRelativePath(document.uri) : document.fileName;
    let context = `
...
`;
      ...
      const visibleRange = new vscode2.Range(startLine, 0, endLine, document.lineAt(endLine).text.length);
...
Workspace: ${folder.name}`;
  ...
  async getProjectStructure() {
    ...
    try {
      ...
      for (const file of files) {
        const relativePath = vscode2.workspace.asRelativePath(file);
...
<<<END_OPERATION>>>`;
    ...
    while ((match = regex.exec(response)) !== null) {
      ...
      if (typeMatch && pathMatch) {
        ...
        operations.push({
          ...
          content: contentMatch ? contentMatch[1] : void 0
        });
      }
    }
    return operations;
  ...
  async applyFileOperations() {
        ...
        switch (op.type) {
          ...
          case "edit":
            if (op.content !== void 0) {
              ...
              if (dirPath) {
                const dirUri = vscode2.Uri.joinPath(workspaceFolder.uri, dirPath);
                try {
                  await vscode2.workspace.fs.createDirectory(dirUri);
                } catch {
                }
              }
              await vscode2.workspace.fs.writeFile(fileUri, Buffer.from(op.content, "utf8"));
              successCount++;
            }
            break;
          case "delete":
    ...
    this.panel.webview.postMessage({ command: "operationsCleared" });
    if (successCount > 0) {
      vscode2.window.showInformationMessage(`Applied ${successCount} file operation(s)`);
    }
    if (errorCount > 0) {
      vscode2.window.showErrorMessage(`Failed to apply ${errorCount} operation(s)`);
    }
  }
  async previewFileOperation(index) {
    ...
    const fileUri = vscode2.Uri.joinPath(workspaceFolder.uri, operation.path);
    try {
      if (operation.type === "create") {
        const emptyUri = vscode2.Uri.parse(`untitled:empty`);
        const proposedUri = vscode2.Uri.parse(`tokamak-preview:${operation.path}`);
        ...
        const disposable = vscode2.workspace.registerTextDocumentContentProvider("tokamak-preview", provider);
        ...
        await vscode2.window.showTextDocument(fileUri, { preview: true });
        vscode2.window.showWarningMessage(`This file will be deleted: ${operation.path}`);
      }
    } catch (error) {
      vscode2.window.showErrorMessage(`Could not preview: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
...
Additional context: ${remainingText}` : slashCommand.prompt;
    ...
    const displayText = attachedFiles.length > 0 || attachedImages.length > 0 ? `${text}${attachedFiles.length > 0 ? `

\u{1F4CE} ${attachedFiles.join(", ")}` : ""}${attachedImages.length > 0 ? `

\u{1F5BC}\uFE0F ${attachedImages.length} images attached (pasted)` : ""}` : text;
...
\uBB38\uC81C\uAC00 \uACC4\uC18D\uB418\uBA74 \uC124\uC815\uC744 \uD655\uC778\uD574\uC8FC\uC138\uC694.`
  ...
  async insertCodeToEditor(code) {
    ...
    if (editor) {
      ...
      vscode2.window.showInformationMessage("Code inserted!");
    } else {
      vscode2.window.showWarningMessage("No active editor to insert code into.");
    }
  }
  async runInTerminal(command) {
    let terminal = vscode2.window.terminals.find((t2) => t2.name === "Tokamak");
    if (!terminal) {
      terminal = vscode2.window.createTerminal("Tokamak");
    }
    terminal.show();
  ...
  async loadSkillsFromFolder() {
    ...
    const skillsFolder = vscode2.Uri.joinPath(workspaceFolder.uri, ".tokamak", "skills");
    const skills = [];
    try {
      const files = await vscode2.workspace.fs.readDirectory(skillsFolder);
      for (const [fileName, fileType] of files) {
        if (fileType === vscode2.FileType.File && fileName.endsWith(".md")) {
          const filePath = vscode2.Uri.joinPath(skillsFolder, fileName);
          try {
            const content = await vscode2.workspace.fs.readFile(filePath);
        ...
        function renderSessions(sessions, currentId) {
            historyList.innerHTML = sessions.map(s => {
                ...
                return '<div class="session-item ' + activeClass + '" data-id="' + s.id + '">' +
                       ...
                       '<span class="delete-session" data-id="' + s.id + '">\u{1F5D1}\uFE0F</span>' +
                       '</div>';
            }).join('');
...
var InlineCompletionProvider = class {
  ...
  async provideInlineCompletionItems(document, position, context, token) {
    ...
    if (context.triggerKind === vscode3.InlineCompletionTriggerKind.Invoke) {
  ...
  async getCompletion(document, position, token) {
    try {
      const prefixRange = new vscode3.Range(
        new vscode3.Position(Math.max(0, position.line - 50), 0),
        position
      );
      const suffixRange = new vscode3.Range(
        position,
        new vscode3.Position(Math.min(document.lineCount - 1, position.line + 20), 0)
      );
      ...
      const item = new vscode3.InlineCompletionItem(
        cleanedCompletion,
...
function getOutputChannel() {
  if (!outputChannel) {
    outputChannel = vscode4.window.createOutputChannel("Tokamak AI");
  }
...
async function explainCode() {
  ...
  const selection = editor.selection;
  if (selection.isEmpty) {
    vscode4.window.showWarningMessage("Please select code to explain");
    return;
  }
  if (!isConfigured()) {
    const configured = await promptForConfiguration();
...
\`\`\``
    ...
    vscode4.window.showErrorMessage(`Failed to explain code: ${errorMessage}`);
  }
}
async function refactorCode() {
  ...
  const selection = editor.selection;
  if (selection.isEmpty) {
    vscode4.window.showWarningMessage("Please select code to refactor");
    return;
  }
  if (!isConfigured()) {
    const configured = await promptForConfiguration();
  ...
  const selectedText = editor.document.getText(selection);
  const language = editor.document.languageId;
  const refactorType = await vscode4.window.showQuickPick(
    [
  ...
  if (refactorType.label === "Custom") {
    const custom = await vscode4.window.showInputBox({
...
Provide only the refactored code, nothing else:`
    ...
    if (!refactoredCode) {
      vscode4.window.showWarningMessage("No refactored code received");
      return;
    }
    ...
    if (action === "Apply Changes") {
      ...
      vscode4.window.showInformationMessage("Code refactored successfully!");
    ...
    const errorMessage = error instanceof Error ? error.message : "Unknown error";
    vscode4.window.showErrorMessage(`Failed to refactor code: ${errorMessage}`);
  }
}
...
function activate(context) {
  ...
  context.subscriptions.push(
    vscode5.commands.registerCommand("tokamak.openChat", () => {
      ChatPanel.createOrShow(context.extensionUri);
    })
  );
  context.subscriptions.push(
    vscode5.commands.registerCommand("tokamak.explainCode", explainCode)
  );
  context.subscriptions.push(
    vscode5.commands.registerCommand("tokamak.refactorCode", refactorCode)
  );
  context.subscriptions.push(
    vscode5.commands.registerCommand("tokamak.clearChat", () => {
      if (ChatPanel.currentPanel) {
        ChatPanel.currentPanel.clearChat();
        vscode5.window.showInformationMessage("Chat history cleared");
      }
    })
  );
  context.subscriptions.push(
    vscode5.commands.registerCommand("tokamak.sendToChat", () => {
      const editor = vscode5.window.activeTextEditor;
      if (!editor) {
        vscode5.window.showWarningMessage("No active editor");
        return;
      }
      const selection = editor.selection;
      if (selection.isEmpty) {
        vscode5.window.showWarningMessage("No code selected");
        return;
      }
      const selectedText = editor.document.getText(selection);
      const relativePath = vscode5.workspace.asRelativePath(editor.document.uri);
  ...
  context.subscriptions.push(
    vscode5.commands.registerCommand("tokamak.initSkills", async () => {
      const workspaceFolder = vscode5.workspace.workspaceFolders?.[0];
      if (!workspaceFolder) {
        vscode5.window.showErrorMessage("No workspace folder open");
        return;
      }
      const skillsFolder = vscode5.Uri.joinPath(workspaceFolder.uri, ".tokamak", "skills");
      const defaultSkills = {
        "explain.md": `---
description: Explain the selected code in detail
---
...
Be constructive and specific in your feedback.`
      ...
      try {
        await vscode5.workspace.fs.createDirectory(skillsFolder);
        for (const [fileName, content] of Object.entries(defaultSkills)) {
          const fileUri = vscode5.Uri.joinPath(skillsFolder, fileName);
          await vscode5.workspace.fs.writeFile(fileUri, Buffer.from(content, "utf8"));
        }
        vscode5.window.showInformationMessage(
          `Skills folder created at .tokamak/skills/ with ${Object.keys(defaultSkills).length} default skills`
        );
        const readmeUri = vscode5.Uri.joinPath(skillsFolder, "explain.md");
        await vscode5.window.showTextDocument(readmeUri);
      } catch (error) {
        vscode5.window.showErrorMessage(`Failed to create skills folder: ${error}`);
      }
    })
  );
  context.subscriptions.push(
