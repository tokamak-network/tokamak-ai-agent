...
function getClient() {
  ...
  if (!clientInstance || needsRecreation(settings)) {
    clientInstance = new openai_default({
...
var Planner = class {
  ...
  parsePlan(text) {
    ...
    for (const line of lines) {
      const stepMatch = line.match(/^[\s\d.]*[-*]?\s*\[\s*[x ]\s*\]\s*(.+)/i) || line.match(/^\d+\.\s*(.+)/);
      if (stepMatch) {
        if (currentStep) {
        ...
        let dependsOn;
        if (dependsMatch) {
          dependsOn = dependsMatch[1].split(",").map((s2) => s2.trim());
    ...
    if (currentStep) {
      if (actionBuffer) {
        let cleanedAction = actionBuffer.trim().replace(/^```json\s*|^```\s*/g, "").replace(/```$/g, "");
   ...
   */
  async replan(currentPlan, newContext, streamFn) {
    const planSummary = currentPlan.map(
      (step, idx) => `${idx + 1}. [${step.status}] ${step.description}${step.result ? `
...
`;
    ...
    const completedSteps = currentPlan.filter((s2) => s2.status === "done");
    for (const completed of completedSteps) {
      const matchingNew = newPlan.find(
        (n2) => n2.description.toLowerCase().includes(completed.description.toLowerCase().substring(0, 30))
      );
      if (matchingNew) {
...
var Executor = class {
  ...
  async execute(action) {
    ...
    switch (action.type) {
      ...
      case "multi_write":
        return await this.multiWrite(action.payload);
      case "read":
  ...
  async writeFile(path, content) {
    ...
    const fileUri = vscode2.Uri.joinPath(workspaceFolder.uri, path);
    const edit = new vscode2.WorkspaceEdit();
    try {
      if (content.includes("<<<<<<< SEARCH")) {
        const existingData = await vscode2.workspace.fs.readFile(fileUri);
        ...
        if (anyApplied) {
          ...
          edit.replace(fileUri, new vscode2.Range(new vscode2.Position(0, 0), new vscode2.Position(docLines + 1, 0)), currentContent);
          const success = await vscode2.workspace.applyEdit(edit);
          if (success) {
            return `Successfully updated ${path} via WorkspaceEdit.`;
          } else {
            await vscode2.workspace.fs.writeFile(fileUri, Buffer.from(currentContent, "utf8"));
        ...
        try {
          ...
          const fullRange = new vscode2.Range(doc.positionAt(0), doc.positionAt(doc.getText().length));
          edit.replace(fileUri, fullRange, content);
        } catch {
          edit.createFile(fileUri, { overwrite: true });
          edit.insert(fileUri, new vscode2.Position(0, 0), content);
        }
        const success = await vscode2.workspace.applyEdit(edit);
        if (success) {
          return `Successfully wrote to ${path}`;
        } else {
          await vscode2.workspace.fs.writeFile(fileUri, Buffer.from(content, "utf8"));
          return `Wrote to ${path} via FileSystem API.`;
        }
      }
    } catch (error) {
      const newFileEdit = new vscode2.WorkspaceEdit();
      newFileEdit.createFile(fileUri, { overwrite: true });
      newFileEdit.insert(fileUri, new vscode2.Position(0, 0), content);
      await vscode2.workspace.applyEdit(newFileEdit);
  ...
  async readFile(path) {
    ...
    if (!workspaceFolder) throw new Error("No workspace folder open");
    const fileUri = vscode2.Uri.joinPath(workspaceFolder.uri, path);
    const data = await vscode2.workspace.fs.readFile(fileUri);
  ...
  async deleteFile(path) {
    ...
    if (!workspaceFolder) throw new Error("No workspace folder open");
    const fileUri = vscode2.Uri.joinPath(workspaceFolder.uri, path);
    await vscode2.workspace.fs.delete(fileUri);
  ...
  async runTerminal(command) {
    ...
    return new Promise((resolve) => {
      ...
      exec(command, { cwd, timeout: 3e4, maxBuffer: 1024 * 1024 }, (error, stdout, stderr) => {
        let result = "";
        if (stdout) {
          result += `[STDOUT]
${stdout}
...
`;
   ...
   */
  async executeAtomicMultiWrite(operations, workspaceFolder) {
    const edit = new vscode2.WorkspaceEdit();
    ...
    try {
      for (const op of operations) {
        if (op.operation === "edit" || op.operation === "delete") {
          try {
            const fileUri = vscode2.Uri.joinPath(workspaceFolder.uri, op.path);
            const content = await vscode2.workspace.fs.readFile(fileUri);
      ...
      for (const op of operations) {
        ...
        switch (op.operation) {
          case "create":
            ...
            edit.insert(fileUri, new vscode2.Position(0, 0), op.content);
            break;
          case "edit":
            if (op.content.includes("<<<<<<< SEARCH")) {
              ...
              if (anyApplied) {
                ...
                edit.replace(
                  ...
                  new vscode2.Position(docLines + 1, 0),
                  currentContent
                );
              } else {
      ...
      if (backupMap.size > 0) {
        const rollbackEdit = new vscode2.WorkspaceEdit();
        ...
        await vscode2.workspace.applyEdit(rollbackEdit);
      }
   ...
   */
  async executeNonAtomicMultiWrite(operations, workspaceFolder) {
    const results = [];
    let successCount = 0;
    let failCount = 0;
    for (const op of operations) {
...
var Observer = class {
  ...
  async getDiagnostics(targetFiles) {
    ...
    for (const [uri, fileDiagnostics] of diagnostics) {
      ...
      if (targetFiles && targetFiles.length > 0) {
        const isTarget = targetFiles.some((f2) => filePath.endsWith(f2));
        if (!isTarget) continue;
      }
      for (const diag of fileDiagnostics) {
        if (diag.severity === vscode3.DiagnosticSeverity.Error || diag.severity === vscode3.DiagnosticSeverity.Warning) {
          let codeValue = diag.code;
          if (codeValue && typeof codeValue === "object") {
            codeValue = codeValue.value;
          }
          result.push({
            file: vscode3.workspace.asRelativePath(uri),
            line: diag.range.start.line + 1,
            message: diag.message,
            severity: diag.severity === vscode3.DiagnosticSeverity.Error ? "Error" : "Warning",
            code: codeValue
          });
        }
      }
  ...
  formatDiagnostics(diags) {
    ...
    return diags.map(
      (d2) => `[${d2.severity}] ${d2.file}:${d2.line} - ${d2.message}${d2.code ? ` (${d2.code})` : ""}`
    ).join("\n");
  }
};

// src/agent/searcher.ts
...
var Searcher = class {
  ...
  async searchRelevantFiles(query) {
    ...
    if (activeEditor) {
      const relPath = vscode4.workspace.asRelativePath(activeEditor.document.uri);
      results.set(relPath, { path: relPath, score: 20, reason: "Active context" });
    }
    const keywords = this.extractKeywords(query);
    for (const keyword of keywords) {
      const nameMatches = await vscode4.workspace.findFiles(`**/*${keyword}*`, "**/node_modules/**", 10);
      for (const uri of nameMatches) {
        const relPath = vscode4.workspace.asRelativePath(uri);
    ...
    return Array.from(results.values()).sort((a2, b2) => b2.score - a2.score).slice(0, 15);
  }
  /**
   * 파일 내용에서 키워드를 검색합니다.
   */
  async searchInFileContents(keyword, results) {
    try {
      const codeFiles = await vscode4.workspace.findFiles(
        "**/*.{ts,tsx,js,jsx,py,go,java,cpp,c,h}",
        "**/node_modules/**",
        30
        // 성능을 위해 최대 30개 파일로 제한
      );
  ...
  extractKeywords(query) {
    return query.split(/\s+/).filter((w2) => w2.length > 2).map((w2) => w2.replace(/[^a-zA-Z0-9가-힣_]/g, "")).filter((w2) => w2.length > 0).slice(0, 5);
  }
};
...
\uCF54\uB4DC \uB0B4\uC6A9:
${content.slice(0, 4e3)} // \uC0C1\uC704 \uC77C\uBD80\uB9CC \uC0AC\uC6A9\uD558\uC5EC \uC694\uC57D \uC694\uCCAD (\uD1A0\uD070 \uC808\uC57D)
`;
    try {
      let summary = "";
...
// src/agent/dependencyAnalyzer.ts
...
var DependencyAnalyzer = class {
  ...
  async analyzeFile(path) {
    ...
    try {
      const fileUri = vscode5.Uri.joinPath(workspaceFolder.uri, path);
      const content = await vscode5.workspace.fs.readFile(fileUri);
      const text = Buffer.from(content).toString("utf8");
      const imports = this.extractImports(text, path);
      const exports2 = this.extractExports(text);
      return {
        path,
        imports,
        exports: exports2,
  ...
  extractExports(content) {
    ...
    for (const pattern of exportPatterns) {
      ...
      while ((match = pattern.exec(content)) !== null) {
        if (match[1]) {
          const items = match[1].split(",").map((s2) => s2.trim().split(/\s+as\s+/)[0]);
          exports2.push(...items);
        }
      }
    }
  ...
  resolveImportPath(importPath, fromPath) {
    ...
    for (const part of parts) {
      if (part === "..") {
        const dirs = resolved.split("/").filter((d2) => d2);
        if (dirs.length > 0) {
          dirs.pop();
          resolved = dirs.join("/");
        }
...
`;
  ...
  async handleExecution() {
    ...
    if (!step) {
      const allDone = this.plan.every((s2) => s2.status === "done");
      if (allDone) {
        await this.transitionTo("Done");
      } else {
...
`;
      ...
      if (step.action) {
        try {
          ...
          console.warn("[AgentEngine] Failed to parse action JSON, falling back to raw write.", e2);
  ...
  getNextExecutableStep() {
    return this.plan.find((step) => {
      ...
      return step.dependsOn.every((depId) => {
        const depStep = this.plan.find((s2) => s2.id === depId);
  ...
  async handleObservation() {
    ...
    const errors = this.lastDiagnostics.filter((d2) => d2.severity === "Error");
    if (errors.length > 0) {
...
var ChatPanel = class _ChatPanel {
  ...
  static setContext(context) {
    _ChatPanel.extensionContext = context;
  }
    ...
    switch (message.command) {
      ...
      case "getSessions":
        this.panel.webview.postMessage({
          ...
          sessions: this.sessions.map((s2) => ({ id: s2.id, title: s2.title, timestamp: s2.timestamp, mode: s2.mode })),
          currentSessionId: this.currentSessionId
        });
        break;
      case "loadSession":
        const session = this.sessions.find((s2) => s2.id === message.sessionId);
        if (session) {
          this.saveChatHistory();
        ...
        break;
      case "deleteSession":
        this.sessions = this.sessions.filter((s2) => s2.id !== message.sessionId);
      ...
      case "rejectOperation":
        if (message.index !== void 0 && message.index >= 0 && message.index < this.pendingOperations.length) {
  ...
  static createOrShow(extensionUri) {
    const column = vscode6.ViewColumn.Beside;
    ...
    _ChatPanel.currentPanel = new _ChatPanel(panel, extensionUri);
  }
  clearChat() {
    this.chatHistory = [];
    this.pendingOperations = [];
  ...
  saveChatHistory() {
    if (_ChatPanel.extensionContext) {
      if (!this.currentSessionId) {
        this.currentSessionId = Date.now().toString();
      }
      const existingIndex = this.sessions.findIndex((s2) => s2.id === this.currentSessionId);
      let title = "New Conversation";
      const firstUserMessage = this.chatHistory.find((m2) => m2.role === "user");
      if (firstUserMessage) {
        let content = "";
        if (typeof firstUserMessage.content === "string") {
          content = firstUserMessage.content;
        } else if (Array.isArray(firstUserMessage.content)) {
          const textPart = firstUserMessage.content.find((p2) => p2.type === "text");
      ...
      _ChatPanel.extensionContext.workspaceState.update("tokamak.sessions", this.sessions);
      _ChatPanel.extensionContext.workspaceState.update("tokamak.currentSessionId", this.currentSessionId);
    }
  }
  restoreChatHistory() {
    if (_ChatPanel.extensionContext) {
      this.sessions = _ChatPanel.extensionContext.workspaceState.get("tokamak.sessions") || [];
      this.currentSessionId = _ChatPanel.extensionContext.workspaceState.get("tokamak.currentSessionId");
      if (this.currentSessionId) {
        const currentSession = this.sessions.find((s2) => s2.id === this.currentSessionId);
        if (currentSession) {
  ...
  async exportSession(sessionId) {
    const session = this.sessions.find((s2) => s2.id === sessionId);
    if (!session) {
      vscode6.window.showErrorMessage("Session not found");
      return;
    }
    try {
      const exportData = {
        ...
        messages: session.messages.map((msg) => ({
          role: msg.role,
          content: typeof msg.content === "string" ? msg.content : Array.isArray(msg.content) ? msg.content.map(
            (item) => item.type === "text" ? item.text : item
          ).join("") : JSON.stringify(msg.content)
        }))
      };
      const jsonContent = JSON.stringify(exportData, null, 2);
      const uri = await vscode6.window.showSaveDialog({
        defaultUri: vscode6.Uri.file(`tokamak-chat-${session.title.replace(/[^a-z0-9]/gi, "-")}-${sessionId}.json`),
        filters: {
          "JSON": ["json"],
          "All Files": ["*"]
        },
      ...
      if (uri) {
        await vscode6.workspace.fs.writeFile(uri, Buffer.from(jsonContent, "utf8"));
        vscode6.window.showInformationMessage(`Conversation exported to ${uri.fsPath}`);
      }
    } catch (error) {
      vscode6.window.showErrorMessage(`Failed to export session: ${error}`);
    }
  }
  ...
  async handleModelChange(model) {
    ...
    vscode6.window.showInformationMessage(`Model changed to: ${model}`);
  }
  async searchFiles(query) {
    ...
    try {
      ...
      for (const file of files) {
        const relPath = vscode6.workspace.asRelativePath(file);
        results.push({
          path: relPath,
      ...
      this.panel.webview.postMessage({ command: "fileSearchResults", files: results.sort((a2, b2) => a2.path.length - b2.path.length).slice(0, 50) });
    } catch {
      this.panel.webview.postMessage({ command: "fileSearchResults", files: [] });
    }
  }
  async openFile(path) {
    try {
      const workspaceFolder = vscode6.workspace.workspaceFolders?.[0];
      if (workspaceFolder) {
        const fileUri = vscode6.Uri.joinPath(workspaceFolder.uri, path);
        const stat = await vscode6.workspace.fs.stat(fileUri);
        if (stat.type === vscode6.FileType.Directory) {
          await vscode6.commands.executeCommand("revealInExplorer", fileUri);
        } else {
          await vscode6.window.showTextDocument(fileUri);
        }
      }
    } catch {
      vscode6.window.showErrorMessage(`Could not open: ${path}`);
    }
  }
  async resolveFilePath(uriString) {
    try {
      ...
      if (filePath.startsWith("file://")) {
        const uri = vscode6.Uri.parse(filePath);
        filePath = uri.fsPath;
      }
      ...
      try {
        const stat = await vscode6.workspace.fs.stat(fileUri);
        if (stat.type === vscode6.FileType.File || stat.type === vscode6.FileType.Directory) {
          const fileName = filePath.split(/[/\\]/).pop() || filePath;
          this.panel.webview.postMessage({
            command: "fileDropped",
            path: filePath,
            name: fileName,
            isDir: stat.type === vscode6.FileType.Directory
          });
        }
      } catch {
      }
  ...
  async getFileContent(relativePath) {
    try {
      ...
      const uri = vscode6.Uri.joinPath(workspaceFolder.uri, relativePath);
      const stat = await vscode6.workspace.fs.stat(uri);
      if (stat.type === vscode6.FileType.Directory) {
        const files = await vscode6.workspace.findFiles(
          new vscode6.RelativePattern(uri, "**/*"),
          "**/node_modules/**",
          100
        );
        let structure = `
...
`;
        const folderTree = [];
        for (const file of files) {
          folderTree.push(`- ${vscode6.workspace.asRelativePath(file)}`);
        }
      ...
      const limitedContent = truncated ? lines.slice(0, maxLines).join("\n") + "\n... (truncated)" : content;
      return `
...
`;
  ...
  getEditorContext() {
    ...
    const workspaceFolder = vscode6.workspace.workspaceFolders?.[0];
    const relativePath = workspaceFolder ? vscode6.workspace.asRelativePath(document.uri) : document.fileName;
    let context = `
...
`;
      ...
      const visibleRange = new vscode6.Range(startLine, 0, endLine, document.lineAt(endLine).text.length);
  ...
  async getProjectStructure() {
    ...
    try {
      ...
      for (const file of files) {
        ...
        for (let i2 = 0; i2 < parts.length; i2++) {
          ...
          if (!tree.has(parentPath)) {
            tree.set(parentPath, /* @__PURE__ */ new Set());
          }
          tree.get(parentPath).add(parts[i2]);
        }
      }
      const buildTreeString = (path, indent) => {
        ...
        const sortedChildren = Array.from(children).sort((a2, b2) => {
          const aIsDir = tree.has(path ? `${path}/${a2}` : a2);
          const bIsDir = tree.has(path ? `${path}/${b2}` : b2);
          if (aIsDir && !bIsDir) return -1;
          if (!aIsDir && bIsDir) return 1;
          return a2.localeCompare(b2);
        });
        let result = "";
        for (let i2 = 0; i2 < sortedChildren.length; i2++) {
          const child = sortedChildren[i2];
          const childPath = path ? `${path}/${child}` : child;
          const isDir = tree.has(childPath);
          const isLast = i2 === sortedChildren.length - 1;
          const prefix = isLast ? "\u2514\u2500\u2500 " : "\u251C\u2500\u2500 ";
...
<<<END_OPERATION>>>`;
  ...
  async applyFileOperations() {
    ...
    for (const op of this.pendingOperations) {
      try {
        const fileUri = vscode6.Uri.joinPath(workspaceFolder.uri, op.path);
        switch (op.type) {
          case "create":
            if (op.content !== void 0) {
              edit.createFile(fileUri, { overwrite: true, ignoreIfExists: false });
              edit.insert(fileUri, new vscode6.Position(0, 0), op.content);
              successCount++;
            }
            break;
          case "edit":
            if (op.content !== void 0) {
              const existingData = await vscode6.workspace.fs.readFile(fileUri);
              let currentContent = Buffer.from(existingData).toString("utf8");
              if (op.content.includes("<<<<<<< SEARCH")) {
                const blocks = op.content.split(/>>>>>>> REPLACE\s*/);
                let anyApplied = false;
                ...
                if (anyApplied) {
                  const docLines = currentContent.split("\n").length;
                  edit.replace(fileUri, new vscode6.Range(new vscode6.Position(0, 0), new vscode6.Position(docLines + 1, 0)), currentContent);
                  successCount++;
                } else {
                ...
                const docLines = currentContent.split("\n").length;
                edit.replace(fileUri, new vscode6.Range(new vscode6.Position(0, 0), new vscode6.Position(docLines + 1, 0)), op.content);
                successCount++;
              }
            }
            break;
    ...
    if (success) {
      if (successCount > 0) {
        vscode6.window.showInformationMessage(`Successfully applied ${successCount} file operation(s).`);
      }
    } else {
      console.error("[ChatPanel] WorkspaceEdit failed. Check for read-only files or conflicting edits.");
      if (successCount > 0) {
        vscode6.window.showErrorMessage(`Failed to apply ${successCount} operation(s) via WorkspaceEdit. Please verify file permissions.`);
      } else if (errorCount > 0) {
        vscode6.window.showErrorMessage(`Failed to stage ${errorCount} operation(s). Check the console (Developer Tools) for details.`);
      }
    }
    this.pendingOperations = [];
    this.panel.webview.postMessage({ command: "operationsCleared" });
  ...
  async previewFileOperation(index) {
    ...
    const fileUri = vscode6.Uri.joinPath(workspaceFolder.uri, operation.path);
    try {
      if (operation.type === "create") {
        const emptyUri = vscode6.Uri.parse("untitled:empty");
        const proposedUri = vscode6.Uri.parse(`tokamak-preview:${operation.path}`);
        ...
        const disposable = vscode6.workspace.registerTextDocumentContentProvider("tokamak-preview", provider);
        await vscode6.commands.executeCommand("vscode.diff", emptyUri, proposedUri, `[CREATE] ${operation.path}`);
        setTimeout(() => disposable.dispose(), 5e3);
        ...
        try {
          ...
          if (proposedContent.includes("<<<<<<< SEARCH")) {
            ...
            for (const block of blocks) {
              ...
              if (searchContent !== void 0 && replaceContent !== void 0) {
          ...
          const disposable = vscode6.workspace.registerTextDocumentContentProvider("tokamak-preview", provider);
          const proposedUri = vscode6.Uri.parse(`tokamak-preview:${operation.path}`);
          await vscode6.commands.executeCommand("vscode.diff", fileUri, proposedUri, `[EDIT] ${operation.path}`);
          setTimeout(() => disposable.dispose(), 5e3);
        } catch (error) {
          vscode6.window.showErrorMessage(`Preview failed: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
      } else if (operation.type === "delete") {
        await vscode6.window.showTextDocument(fileUri, { preview: true });
        vscode6.window.showWarningMessage(`This file will be deleted: ${operation.path}`);
      }
    } catch (error) {
      vscode6.window.showErrorMessage(`Could not preview: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
...
\`\`\``
    ...
    if (slashCommand) {
      processedText = remainingText ? `${slashCommand.prompt} 

Additional context: ${remainingText} ` : slashCommand.prompt;
    }
    let fileContexts = "";
...
\u{1F5BC}\uFE0F ${attachedImages.length} images attached (pasted)`;
    }
        ...
        if (readOps.length > 0) {
          ...
          for (const op of readOps) {
            this.panel.webview.postMessage({
              command: "addMessage",
              role: "assistant",
              content: `\u{1F50D} *Reading file: ${op.path}*`
            });
            const content2 = await this.getFileContent(op.path);
            readResults += content2;
          }
...
\uBB38\uC81C\uAC00 \uACC4\uC18D\uB418\uBA74 \uC124\uC815\uC744 \uD655\uC778\uD574\uC8FC\uC138\uC694.`
  ...
  async insertCodeToEditor(code) {
    ...
    if (editor) {
      ...
      vscode6.window.showInformationMessage("Code inserted!");
    } else {
      vscode6.window.showWarningMessage("No active editor to insert code into.");
    }
  }
  async runInTerminal(command) {
    ...
    let terminal = vscode6.window.terminals.find((t2) => t2.name === "Tokamak");
    if (!terminal) {
      terminal = vscode6.window.createTerminal("Tokamak");
    }
    terminal.show();
    terminal.sendText(command);
    vscode6.window.showInformationMessage(`Running: ${command}`);
    try {
      const { exec } = require("child_process");
      const cwd = workspaceFolder.uri.fsPath;
...
${result}`
      ...
      const errorMsg = error instanceof Error ? error.message : "Unknown error";
      vscode6.window.showErrorMessage(`Command failed: ${errorMsg}`);
    }
  }
  stopGeneration() {
    if (this.currentAbortController) {
  ...
  async loadSkillsFromFolder() {
    ...
    const skillsFolder = vscode6.Uri.joinPath(workspaceFolder.uri, ".tokamak", "skills");
    const skills = [];
    try {
      const files = await vscode6.workspace.fs.readDirectory(skillsFolder);
      for (const [fileName, fileType] of files) {
        if (fileType === vscode6.FileType.File && fileName.endsWith(".md")) {
          const filePath = vscode6.Uri.joinPath(skillsFolder, fileName);
          try {
            const content = await vscode6.workspace.fs.readFile(filePath);
            const text = Buffer.from(content).toString("utf8");
  ...
  async getAllSkills() {
    ...
    const fileSkillNames = new Set(fileSkills.map((s2) => s2.name));
    const builtinSkills = BUILTIN_SKILLS.filter((s2) => !fileSkillNames.has(s2.name));
    return [...fileSkills, ...builtinSkills];
  }
  async searchSlashCommands(query) {
    const allSkills = await this.getAllSkills();
    const filtered = allSkills.filter(
      (cmd) => cmd.name.toLowerCase().includes(query.toLowerCase()) || cmd.description.toLowerCase().includes(query.toLowerCase())
    );
    this.panel.webview.postMessage({
...
<body>
    ...
    <div id="history-panel">
        <div id="history-header">
            <h4>Chat History</h4>
            <button id="close-history" style="background:none; border:none; color:inherit; cursor:pointer; font-size:1.4em;">\xD7</button>
        </div>
    ...
    <div id="header">
        <div id="header-top">
            <button id="history-btn" title="Past Conversations">\u{1F552} History</button>
        ...
        <div id="mode-tabs">
            <button class="mode-tab active" data-mode="ask">\u{1F4AC} Ask</button>
            <button class="mode-tab" data-mode="plan">\u{1F4CB} Plan</button>
            <button class="mode-tab" data-mode="agent">\u{1F916} Agent</button>
        </div>
    ...
    <div id="plan-panel">
        <div class="plan-header">
            <h4>\u{1F4CB} Implementation Plan</h4>
    ...
    <div id="input-container">
        ...
        <div id="drop-zone" class="drop-zone">
            ...
            <div class="drop-overlay">\u{1F4C1} Drop files here</div>
        </div>
        <div class="hint">\u{1F4A1} Type <strong>/</strong> for commands, <strong>@</strong> to attach files</div>
    </div>
            ...
            tag.innerHTML = '<img src="' + base64Data + '"><span class="remove-img">\xD7</span>';
            ...
            tag.innerHTML = \`
            <span class="icon">\${isDir ? '\u{1F4C1}' : '\u{1F4C4}'}</span>
            <span class="file-name" data-path="\${filePath}">\${fileName}</span>
            <span class="remove-btn" data-path="\${filePath}">\xD7</span>
            \`;
            ...
            <span class="icon">\${file.isDir ? '\u{1F4C1}' : '\u{1F4C4}'}</span>
            ...
            '<button class="reject-item-btn" data-index="' + index + '" title="Reject this change">\xD7</button>' +
            '</div>'
            ).join('');
            ...
            '<span class="export-session" data-id="' + s.id + '" title="Export conversation">\u{1F4E5}</span>' +
            '<span class="delete-session" data-id="' + s.id + '" title="Delete conversation">\u{1F5D1}\uFE0F</span>' +
            '</div>' +
            '</div>';
            }).join('');
...
var InlineCompletionProvider = class {
  ...
  async provideInlineCompletionItems(document, position, context, token) {
    ...
    if (context.triggerKind === vscode7.InlineCompletionTriggerKind.Invoke) {
  ...
  async getCompletion(document, position, token) {
    try {
      const prefixRange = new vscode7.Range(
        new vscode7.Position(Math.max(0, position.line - 50), 0),
        position
      );
      const suffixRange = new vscode7.Range(
        position,
        new vscode7.Position(Math.min(document.lineCount - 1, position.line + 20), 0)
      );
      ...
      const item = new vscode7.InlineCompletionItem(
        cleanedCompletion,
...
function getOutputChannel() {
  if (!outputChannel) {
    outputChannel = vscode8.window.createOutputChannel("Tokamak AI");
  }
...
async function explainCode() {
  ...
  const selection = editor.selection;
  if (selection.isEmpty) {
    vscode8.window.showWarningMessage("Please select code to explain");
    return;
  }
  if (!isConfigured()) {
    const configured = await promptForConfiguration();
  ...
  output.show(true);
  output.appendLine("\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501");
  output.appendLine("\u{1F4D6} Code Explanation");
  output.appendLine("\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501");
  output.appendLine("");
  try {
    const messages = [
...
\`\`\``
    ...
    vscode8.window.showErrorMessage(`Failed to explain code: ${errorMessage}`);
  }
}
async function refactorCode() {
  ...
  const selection = editor.selection;
  if (selection.isEmpty) {
    vscode8.window.showWarningMessage("Please select code to refactor");
    return;
  }
  if (!isConfigured()) {
    const configured = await promptForConfiguration();
  ...
  const selectedText = editor.document.getText(selection);
  const language = editor.document.languageId;
  const refactorType = await vscode8.window.showQuickPick(
    [
  ...
  if (refactorType.label === "Custom") {
    const custom = await vscode8.window.showInputBox({
...
Provide only the refactored code, nothing else:`
    ...
    if (!refactoredCode) {
      vscode8.window.showWarningMessage("No refactored code received");
      return;
    }
    ...
    if (action === "Apply Changes") {
      ...
      vscode8.window.showInformationMessage("Code refactored successfully!");
    } else if (action === "Show in Output") {
      const output = getOutputChannel();
      output.show(true);
      output.appendLine("\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501");
      output.appendLine(`\u{1F527} Refactored Code (${refactorInstruction})`);
      output.appendLine("\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501");
      output.appendLine("");
    ...
    const errorMessage = error instanceof Error ? error.message : "Unknown error";
    vscode8.window.showErrorMessage(`Failed to refactor code: ${errorMessage}`);
  }
}
...
function activate(context) {
  ...
  context.subscriptions.push(
    vscode9.commands.registerCommand("tokamak.openChat", () => {
      ChatPanel.createOrShow(context.extensionUri);
    })
  );
  context.subscriptions.push(
    vscode9.commands.registerCommand("tokamak.explainCode", explainCode)
  );
  context.subscriptions.push(
    vscode9.commands.registerCommand("tokamak.refactorCode", refactorCode)
  );
  context.subscriptions.push(
    vscode9.commands.registerCommand("tokamak.clearChat", () => {
      if (ChatPanel.currentPanel) {
        ChatPanel.currentPanel.clearChat();
        vscode9.window.showInformationMessage("Chat history cleared");
      }
    })
  );
  context.subscriptions.push(
    vscode9.commands.registerCommand("tokamak.sendToChat", () => {
      ...
      const selectedText = editor.document.getText(selection);
      const relativePath = vscode9.workspace.asRelativePath(editor.document.uri);
  ...
  context.subscriptions.push(
    vscode9.commands.registerCommand("tokamak.initSkills", async () => {
      const workspaceFolder = vscode9.workspace.workspaceFolders?.[0];
      if (!workspaceFolder) {
        vscode9.window.showErrorMessage("No workspace folder open");
        return;
      }
      const skillsFolder = vscode9.Uri.joinPath(workspaceFolder.uri, ".tokamak", "skills");
      const defaultSkills = {
        "explain.md": `---
description: Explain the selected code in detail
---
...
Be constructive and specific in your feedback.`
      ...
      try {
        await vscode9.workspace.fs.createDirectory(skillsFolder);
        for (const [fileName, content] of Object.entries(defaultSkills)) {
          const fileUri = vscode9.Uri.joinPath(skillsFolder, fileName);
          await vscode9.workspace.fs.writeFile(fileUri, Buffer.from(content, "utf8"));
        }
        ...
        const readmeUri = vscode9.Uri.joinPath(skillsFolder, "explain.md");
        await vscode9.window.showTextDocument(readmeUri);
      } catch (error) {
        vscode9.window.showErrorMessage(`Failed to create skills folder: ${error}`);
      }
    })
  );
  context.subscriptions.push(
    vscode9.workspace.onDidChangeConfiguration((e2) => {
      if (e2.affectsConfiguration("tokamak")) {
        resetClient();
      }
    })
  );
