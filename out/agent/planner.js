...
class Planner {
    ...
    async replan(currentPlan, newContext, streamFn) {
        const planSummary = currentPlan.map((step, idx) => `${idx + 1}. [${step.status}] ${step.description}${step.result ? `\n   Result: ${step.result.substring(0, 200)}...` : ''}`).join('\n');
        const prompt = `
...
`;
        ...
        for (const completed of completedSteps) {
            const matchingNew = newPlan.find(n => n.description.toLowerCase().includes(completed.description.toLowerCase().substring(0, 30)));
            if (matchingNew) {
